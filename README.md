...
- **User_Review**
  :한 명의 사용자는 여러개의 리뷰를 등록할 수 있으므로 일대다(One-to-Many) 관계이다.

- **Restaurant_Menu**
  :하나의 식당은 여러 개의 메뉴를 가질 수 있으므로 일대다(One-to-Many) 관계이다.

- **Restaurant_Review**
  :하나의 식당은 여러 개의 리뷰를 가질 수 있으므로 일대다(One-to-Many) 관계이다.

- **Menu_Review**
  :하나의 메뉴에 여러개의 리뷰가 존재하므로, 일대다(One-to-Many) 관계이다.

- **Review_Menu_Res_Mapping**
  :여러 식당에 여러 메뉴에 대한 여러 리뷰가 존재할 수 있으므로 3진 다대다(Many-to-Many) 관계이다.
